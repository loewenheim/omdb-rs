...
impl From<FindResponse> for Movie {
    fn from(find: FindResponse) -> Movie {
        Movie {
            ...
            genre: find.genre.unwrap_or_default().split(", ").map(|s| s.to_owned()).collect(),
            director: find.director.unwrap_or_default().split(", ").map(|s| s.to_owned()).collect(),
            writer: find.writer.unwrap_or_default().split(", ").map(|s| s.to_owned()).collect(),
            actors: find.actors.unwrap_or_default().split(", ").map(|s| s.to_owned()).collect(),
            plot: find.plot.unwrap_or_default(),
            language: find.language.unwrap_or_default(),
            country: find.country.unwrap_or_default().split(", ").map(|s| s.to_owned()).collect(),
